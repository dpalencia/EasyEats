...
class FieldIngredientStream extends StatelessWidget {
  ...
  FieldIngredientStream(this._docRef, this._fieldName);
  @override 
          ...
          itemBuilder: (BuildContext context, int index) {
              return StreamBuilder<DocumentSnapshot>(
                ...
                builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> ingredient) {
                  ...
                  if(ingredient.connectionState == ConnectionState.waiting) return CircularProgressIndicator();
...
/*
class UserIngredientStream extends StatelessWidget {

  @override 
  build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot> (
      // A stream of documentsnapshots from the user.
      stream: Firestore.instance.collection('dishes').document(getTheUserID(context)).snapshots(),
      builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
        // The outer build returns the ListView.
        return SizedBox(
          height: 200,
          child: ListView.builder(
          shrinkWrap: true,
          // The listview is as long as the array of references we're watching.
          itemCount: snapshot.data["kitchenRefs"].length,
          itemBuilder: (BuildContext context, int index) {
              return StreamBuilder<DocumentSnapshot>(
                stream: snapshot.data["kitchenRefs"][index].snapshots(),
                builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> ingredient) {
                  return StreamIngredientTile(Ingredient.fromSnapshot(ingredient.data));
                }
              );
